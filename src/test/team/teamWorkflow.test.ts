...
afterEach((done) => {
  // doesnt clean up for some reason
  User.deleteMany({});
  Team.deleteMany({});
  done;
});
...
describe("Team workflow tests", () => {
  it("should register (which creates one team for the user by default) + login a user and get all teams (1 team that was created by default)", (done) => {
    let user = {
      firstName: "Sarah",
      lastName: "Connor",
